...
M = m*m
# edge_score[i][j][d] is the score when we put piece j in direction d from i
es = np.zeros((M, M, 4), dtype=np.uint32)

def pixel_err(x, y):
    return (int(x[0])-y[0])**2 + (int(x[1])-y[1])**2 + (int(x[2])-y[2])**2

def test_edge(x1, y1, x2, y2, dir):
    if(dir == "LEFT"): return test_edge(x2, y2, x1, y1, "RIGHT")
    elif(dir == "UP"): return test_edge(x2, y2, x1, y1, "DOWN")
...
for i in range(0, M):
    ...
    for j in range(i+1, M):
        es[i][j][0] = test_edge(i//m, i%m, j//m, j%m, "RIGHT")
        es[i][j][1] = test_edge(i//m, i%m, j//m, j%m, "UP")
        es[i][j][2] = test_edge(i//m, i%m, j//m, j%m, "LEFT")
        es[i][j][3] = test_edge(i//m, i%m, j//m, j%m, "DOWN")
